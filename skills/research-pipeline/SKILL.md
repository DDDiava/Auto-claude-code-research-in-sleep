...
description: "Legacy exploratory full pipeline. Runs Workflow 1 (idea discovery) and compatibility implementation/review/writing steps, but submission-grade research must migrate into the Claim-PR truth layer (`researchctl` claim/run/verdict/paper-build). Use `/claim-pr-start` as the default evidence-governed path."
...
It preserves the older orchestration style, but submission-grade state is owned by `researchctl`. Workflow 3 (`/paper-writing`) is optional draft writing only unless `/paper-build` and `audit submission` pass.
...
Do not invoke `/paper-writing` as a submission-grade route unless `/paper-build` has already passed for the relevant merged claims. Without that gate, this stage is draft-only.
